...
class ColumnModel{
	...
	function checkReference($tableList){
		if($this->isReference() && is_string($this->reference[0])){
			...
			$ref_table = is_string($this->reference[0]) ? $this->reference[0] : $this->reference[0]->name;
			return ExceptionTrigger::raise(new NotFoundException(Message::_("reference table [{1}.{2}]",$ref_table,$this->reference[1])));
		}
		return true;
	}
