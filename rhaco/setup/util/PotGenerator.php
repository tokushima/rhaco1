...
class PotGenerator extends Flow{
	...
	function _createPot($path="",$outputpath=""){
		...
		foreach($messageList as $message => $fileList){
			if(strpos($message,'{$') === false){
				foreach(ArrayUtil::arrays($fileList) as $fileline) $outoutsrc .= sprintf("%s\n",$fileline);
				$outoutsrc .= "\n";
	...
	function _massageList($path=""){
		...
		if($path == Rhaco::path() && FileUtil::exist(Rhaco::setuppath("project.xml"))){
			...
			if($tag->set($src,"project")){
				$tag->getIn("session",true);
				$tag->getIn("cookie",true);
				$tag->getIn("define",true);

				foreach($tag->getIn("database",true) as $databaseTag){
				...
				foreach($tag->getIn(array("input","password","text"),true) as $inputTag){
				...
				foreach($tag->getIn("select",true) as $inputTag){
					foreach($inputTag->getIn("description") as $valueTag){
						$messageList[$this->_escape($valueTag->getValue())][] = sprintf("#: setup/project.xml:select");
					}
					foreach($inputTag->getIn("title") as $valueTag){
						$messageList[$this->_escape($valueTag->getValue())][] = sprintf("#: setup/project.xml:select");
					}
					foreach($inputTag->getIn("data") as $valueTag){
						$messageList[$this->_escape($valueTag->getParameter("caption"))][] = sprintf("#: setup/project.xml:select");
					}
				}
				$messageList[trim($tag->getInValue("description"))] = sprintf("#: setup/project.xml:description");
			}
		}
		return $messageList;
	}
	...
	function _marge($path=""){
		...
		if($file->isError()){
			...
			foreach($messageList as $message => $fileList){
				if(strpos($message,'{$') === false){
					foreach(ArrayUtil::arrays($fileList) as $fileline){
						$outoutsrc .= sprintf("%s\n",$fileline);
					}
					$outoutsrc .= "\n";
					$outoutsrc .= sprintf("msgid \"%s\"\n",$message);
