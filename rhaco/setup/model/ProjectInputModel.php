...
class ProjectInputModel{
	...
	function toInstance($inputTag){
		...
		switch(strtolower($inputTag->getName())){
			case "input":
				$input->type		= "input";
				$input->title		= Message::_($inputTag->getInValue("title"));
				$input->description	= Message::_($inputTag->getInValue("description"));
			...
			case "select":
				...
				foreach($inputTag->getIn("data") as $dataTag){
					$input->dataList[Message::_($dataTag->getParameter("caption",$dataTag->getValue()))] = $dataTag->getValue();
				}
		}
