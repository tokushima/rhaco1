	...
	function start($projectxml){
		...
		if(FileUtil::exist($this->projectxml)){
			...
			if(SimpleTag::setof($projectTag,SimpleTag::uncomment($parser->read($this->projectxml)),"project")){
				...
				$this->setDescription(Message::_(trim($projectTag->getInValue("description"))));
			}
		}
		...
		if(Rhaco::constant("CONTEXT_URL") !== null){
			...
			$this->url = preg_replace("/^(.+)\/setup\.php.*$/","\\1",
								isset($_SERVER["SCRIPT_URI"]) ?
									$_SERVER["SCRIPT_URI"] :
									(isset($_SERVER["SERVER_NAME"]) ? "http://".$_SERVER["SERVER_NAME"].((isset($_SERVER["SERVER_PORT"]) && $_SERVER["SERVER_PORT"] != 80) ? ":".$_SERVER["SERVER_PORT"] : "").(isset($_SERVER["REQUEST_URI"]) ? $_SERVER["REQUEST_URI"] : "") : "")
							);
		}
