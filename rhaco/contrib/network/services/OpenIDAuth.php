...
class OpenIDAuth
{
	...
	public function request() {
		...
		if ($this->xrds_document) {
			$this->xrds = $this->parseXRDSBody($this->xrds_document);
	...
	protected function parseXRDS($url) {
		...
		return $this->parseXRDSBody($response);
	}

	protected function parseXRDSBody($response) {
		$ret = array();
	...
	protected function discover_url() {
		...
		if (isset($headers['Content-Type'])) {
			if ($headers['Content-Type'] == 'application/xrds+xml; charset=UTF-8') {
				$this->xrds_document = $response;
				return true;
			}
		}
