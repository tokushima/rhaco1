...
class Rhaco{
	...
	var $def = array();
	var $shutdown = array();
	var $rep = array('http://rhaco.org/repository/1/lib/');
	...
	function repository($rep){
		array_unshift($GLOBALS["__RHACO__"]->rep,$rep);
	}
	function repositorys(){
		return $GLOBALS["__RHACO__"]->rep;
	}
	...
	/**
	 * 評価する
	 * @param string $rhaco_execute_srcOrPath
	 * @param mixed[] $rhaco_execute_variables
	 * @param boolean $rhaco_execute_include
	 */
	...
	function error($errno,$errstr,$errfile,$errline){
		if((version_compare(phpversion(),strval(5)) >= 0) && $errno == E_STRICT){
			...
			if(strpos($errstr,"Strict Standards") !== false) return true;
			if(strpos(strtolower($errstr),"non-static method") !== false) return true;
		}
		return false;
	}
	/**
	...
	/**
	 * $obj が Rhaco::import で読み込まれたクラスか
	 * @param $type
	 * @param $obj
	 */
