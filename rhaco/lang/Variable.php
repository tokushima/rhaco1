...
class Variable{
	 ...
	 * @param mixed $value
	 * @param boolean $to_str 文字列表現で返すか
	 * @return mixed
	 */
	function bool($value=false,$to_str=false){
		/***
			...
			eq("true",Variable::bool(true,true));
			eq("false",Variable::bool(false,true));
		 */		
		$bool = false;
	...
	/**
	 * $variable を boolean として評価し， $base との論理積を取得
	 * @param mixed $varible
	 * @param boolean $base
	 * @return boolean
	 */
	...
	function parseJson($json){
		...
		$json = preg_replace("/[\s]*([,\:\{\}\[\]])[\s]*/","\\1",
						preg_replace_callback("/[\"].*?[\"]/sm",create_function('$m','return str_replace(array(",",":","{","}","[","]"),array("#B#","#C#","#D#","#E#","#F#","#G#"),$m[0]);'),str_replace("\\\"","#A#",trim($json))));
		if(preg_match("/^\"([^\"]*?)\"$/",$json)){
