...
class ObjectUtil{
	 ...
	 * 
	 */
	function sort(&$objects,$property,$order=true){
		/*** unit("lang.ObjectUtilTest"); */
		$ostr = ($order) ? ">=" : "<=";
		uasort($objects,create_function('$a,$b','return ($a->'.$property.' '.$ostr.' $b->'.$property.') ? 1 : -1;'));
		return $objects;
	}
	/**
	 * クラスパスの一覧から対象のメソッドをもつオブジェクトの一覧を返す
	 * @param string[] $args object、またはclass path
	 * @param string[] $methods メソッド名
