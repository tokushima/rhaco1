...
class Validate{
	 ...
	 * @return boolean
	 */
	function isString($variable,$minLength,$maxLength){
		/***
		 * assert(Validate::isString("abc",1,3));
		 * assert(!Validate::isString("abc",4,5));
		 * assert(!Validate::isString(true,2,5));
		 */
		if($minLength <= StringUtil::strlen($variable) && StringUtil::strlen($variable) <= $maxLength){
			return true;
		}
		return false;
	}
	...
	function isEmail($email){
		 ...
		 * 
		 * assert(!Validate::isEmail("aaaaaaa.aaaa_a1234.@ssss.ss.cd"));
		 * assert(!Validate::isEmail("aaaaaaa.aaa@a1234.@ssss.ss.cd"));
		 */
		if(!preg_match('/^[\w\''.preg_quote('./!#$%&*+-=?^_`{|}~','/').']+@(?:[A-Z0-9-]+\.)+[A-Z]{2,6}$/i',$email) 
			|| strlen($email) > 255 || strpos($email,'..') !== false || strpos($email,'.@') !== false){
			return false;
		 }
		 return true;
	}
