...
class DbUtil{
	...
	/**
	 * 最後に実行したSQL
	 * @param boolean $block true の場合トランザクション内で発行した全てのSQLを取得する
	 * @return string
	 */
	 ...
	 * @param TableObjectBase $tableObjectList
			...
			if(Variable::istype("TableObjectBase",$tableObject)){
				if(!ObjectUtil::isMethod($tableObject,"beforeInsert") || false !== $tableObject->beforeInsert($this)){
					...
					if(Variable::istype("TableObjectBase",$obj)){
						if(ObjectUtil::isMethod($obj,"afterInsert") && $obj->afterInsert($this) === false) return false;
						return $obj;
					}
				}
			}
